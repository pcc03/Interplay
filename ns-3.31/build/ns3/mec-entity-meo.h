...
class MecHostEntity;
class MecHostContainer;
class MecAppContainer;
...
//  Istance data
  ...
  //Ptr<Instance> GetIstanceData (std::string appContextId, uint64_t hostId, std::string state);
...
//  void SendUpdateAppContextNotification();
  ...
  Ptr<MecHostContainer> FindBestMecHost(Ptr<AppContext> appContext);

private:
  std::vector<std::string> ue_host_tuple;
