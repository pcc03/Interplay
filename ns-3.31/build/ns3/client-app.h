...
public:
  ...
  void AddMecApp(Ptr<MecApp> mecApp, double mipsCpu);
  void SetContextId(std::string contextId);
  void SetProcessingDensity(double processingDensity);
  bool GetState(void);
  double GetProcessingDensity(void);
  Ptr<UserEquipment> GetUe(void);
  std::map<Ptr<MecApp>,double> GetMecApp(void);
...
private:
  ...
  std::map<Ptr<MecApp>,double> m_mecAppList;
  bool m_state;
